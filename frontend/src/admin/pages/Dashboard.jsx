...
export default function Dashboard() {
  ...
  const { kpis, actions, recent, topDoctors, revenueByDay, topProducts } = data;
  const recentOrders = recent?.orders || [];
  const recentConsultations = recent?.consultations || [];
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <QuickStat
            ...
            value={kpis.avgConsultationTime}
            color="amber"
          />
        </div>

        {/* Action Required - Enhanced Alert */}
        {(actions.pendingDoctors > 0 ||
          actions.pendingOrders > 0 ||
          actions.unpaidConsultations > 0) && (
          <div className="relative overflow-hidden bg-linear-to-r from-red-500 to-pink-500 rounded-2xl shadow-xl">
                ...
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
                    <ActionItem
                      ...
                      value={actions.pendingDoctors}
                      link="/admin/doctors"
                    />
                    <ActionItem
                      label="Pending Orders"
                      value={actions.pendingOrders}
                      link="/admin/orders"
                    />
                    <ActionItem
                      label="Unpaid Consultations"
                      value={actions.unpaidConsultations}
                      link="/admin/consultations"
                    />
                    <ActionItem
                      label="Low Stock Items"
                      value={actions.lowStock}
                      link="/admin/inventory"
                    />
                    <ActionItem
                      label="Pending Refunds"
                      value={actions.pendingRefunds}
                      link="/admin/refunds"
                    />
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            {/* Top Doctors */}
            <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
              <div className="bg-linear-to-r from-amber-400 to-orange-500 p-6">
                <div className="flex items-center gap-3">
              ...
              <div className="p-6 space-y-4">
                {revenueByDay.map((day, idx) => (
                  <RevenueBar
                    ...
                    day={day.day}
                    revenue={day.revenue}
                    max={Math.max(...revenueByDay.map((d) => d.revenue))}
                  />
                ))}
              </div>
            </div>
        ...
        {/* Top Products */}
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
          <div className="bg-linear-to-r from-pink-500 to-rose-500 p-6">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-white/20 backdrop-blur-sm rounded-lg">
...
const KpiCard = ({
  ...
  return (
    <div className="relative group">
      <div
        className="absolute inset-0 bg-linear-to-r opacity-0 group-hover:opacity-100 transition-opacity blur-xl -z-10"
        style={{
      ...
      <div className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all p-6 border border-gray-100">
        <div className="flex items-start justify-between mb-4">
          <div
            className={`p-4 rounded-xl bg-linear-to-br ${bgGradient} shadow-md`}
          >
            <Icon
              className={`w-7 h-7 bg-linear-to-r ${gradient} bg-clip-text text-transparent`}
              style={{ fill: "url(#grad)" }}
            />
          </div>
          <div
...
const QuickStat = ({ icon: Icon, label, value, color }) => {
  ...
  return (
    <div className="bg-white rounded-xl p-4 shadow-md hover:shadow-lg transition-all border border-gray-100">
      <div className="flex items-center gap-3">
        <div
          className={`p-3 rounded-lg bg-linear-to-br ${colors[color]} shadow-md`}
        >
...
const ActionItem = ({ label, value, link }) => (
  <a
    href={link}
    className="block p-4 bg-white/90 backdrop-blur-sm rounded-xl hover:bg-white hover:scale-105 transition-all cursor-pointer shadow-md"
  >
...
const OrderRow = ({ order }) => {
  ...
  const status =
    statusStyles[order.orderStatus || order.status] || statusStyles.pending;

  return (
    <div className="flex items-center justify-between p-4 hover:bg-linear-to-r hover:from-blue-50 hover:to-purple-50 rounded-xl transition-all cursor-pointer group border border-transparent hover:border-blue-200">
      <div className="flex items-center gap-4 flex-1">
        <div className="w-12 h-12 bg-linear-to-br from-blue-400 to-purple-500 rounded-xl flex items-center justify-center shadow-md group-hover:shadow-lg transition-all">
          <Package className="w-6 h-6 text-white" />
        </div>
        <div>
          <p className="font-semibold text-gray-900">{order.patient}</p>
          <p className="text-sm text-gray-500">
            {order.orderId || order.id || "N/A"} • {order.time || "N/A"}
          </p>
        </div>
      </div>
...
const ConsultationRow = ({ consultation }) => {
  ...
  const config = statusConfig[consultation.status] || statusConfig.scheduled;
  const StatusIcon = config.icon;

  return (
    <div className="flex items-center justify-between p-4 hover:bg-linear-to-r hover:from-green-50 hover:to-emerald-50 rounded-xl transition-all cursor-pointer group border border-transparent hover:border-green-200">
      <div className="flex items-center gap-4 flex-1">
        <div className="w-12 h-12 bg-linear-to-br from-green-400 to-emerald-500 rounded-xl flex items-center justify-center shadow-md group-hover:shadow-lg transition-all">
          <Stethoscope className="w-6 h-6 text-white" />
        </div>
        <div>
          <p className="font-semibold text-gray-900">
            {consultation.name || consultation.patient || "Unknown Patient"}
          </p>
          <p className="text-sm text-gray-500">
            {consultation.specialistDoctor?.name ||
              consultation.doctor ||
              "Unknown Doctor"}{" "}
...
const DoctorCard = ({ doctor, rank }) => {
  ...
  return (
    <div className="flex items-center gap-4 p-4 bg-linear-to-r from-amber-50 to-orange-50 rounded-xl hover:shadow-md transition-all border border-amber-100">
      <div
        className={`w-10 h-10 bg-linear-to-br ${
          rankColors[rank] || "from-blue-400 to-blue-500"
        } text-white rounded-full flex items-center justify-center font-bold text-lg shadow-md`}
      >
        {rank}
      </div>
      <div className="flex-1">
  ...
  return (
    <div className="group">
      ...
      <div className="relative w-full bg-gray-100 rounded-full h-3 overflow-hidden">
        <div
          className="absolute inset-0 bg-linear-to-r from-indigo-500 via-purple-500 to-pink-500 rounded-full transition-all duration-700 group-hover:shadow-lg"
...
const ProductCard = ({ product }) => {
  ...
  return (
    <div className="p-5 bg-linear-to-br from-white to-pink-50 border-2 border-pink-100 rounded-xl hover:border-pink-300 hover:shadow-xl transition-all group">
      <div className="flex items-start justify-between mb-4">
        <h4 className="font-bold text-gray-900 line-clamp-2 flex-1">
          {product.name}
        </h4>
