...
const qtyBtn =
  "p-1.5 border border-gray-300 rounded-md hover:bg-gray-100 hover:border-gray-400 transition-all";
...
const Cart = () => {
      ...
      try {
        const categories = [
          ...new Set(
            cartItems
              .map(
                (item) => item?.productId?.category || item?.product?.category
              )
              .filter(Boolean)
          ),
        ];
        ...
        const cartProductIds = cartItems
          .map((item) => item?.productId?._id || item?.product?._id)
          .filter(Boolean);

        const res = await fetch(
          `${import.meta.env.VITE_API_URL || "http://localhost:5000"}/products`
        );
  ...
  if (!cartItems || cartItems.length === 0)
    return (
      <div className="min-h-screen flex items-center justify-center bg-linear-to-br from-gray-50 to-gray-100 py-12">
        <div className="text-center max-w-md mx-auto px-4">
          ...
          <p className="text-gray-500 mb-8">
            Browse our wide selection of medicines and add what you need to get
            started.
          </p>
          <Link
            to="/medicines"
              ...
              <AnimatePresence>
                {cartItems.map((item, index) => {
                  ...
                  return (
                    ...
                    >
                      {/* Product Image */}
                      <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-xl bg-linear-to-br from-gray-100 to-gray-50 flex items-center justify-center overflow-hidden shrink-0 border border-gray-200">
                        <img
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <span className="text-center text-red-800">
                            {`${product.discount * item.quantity} % offer` }
                          </span>
                        </div>
                  ...
                  {relatedProducts.map((product) => (
                    ...
                    >
                      <div className="flex gap-3">
                        <div className="w-20 h-20 rounded-lg bg-linear-to-br from-gray-100 to-gray-50 flex items-center justify-center overflow-hidden shrink-0">
                          <img
                            src={product.image || "/placeholder.png"}
                            alt={product.name}
                            className="w-full h-full object-cover"
          ...
          <div>
            <div className="bg-white rounded-2xl shadow-lg border border-gray-100 sticky top-20">
              ...
              <div className="p-6 space-y-5">
                ...
                {/* Savings Indicator */}
                {discount > 0 && (
                  <div className="bg-linear-to-r from-green-50 to-emerald-50 border border-green-200 rounded-lg p-3 text-center">
